# -*- coding: utf-8 -*-
"""
Created on Wed Sep 02 10:44:27 2015
...
class Differentiate:
    def __init__(self, f, h=1.0E-8):
        if callable(f) and isinstance(h, (float, int)):
            self.f, self.h = f, float(h)
        else:
            raise TypeError('Function must be callable and h must be float number.')
...
class MinMax_v3:
    def __init__(self, func, left, right, n):
        ...
        and isinstance(n, (float, int)):
            ...
            self.dxdy = Differentiate(self.f)
...
def test_MinMax_v3():
    ...
    if success:
        print 'Test successful!'
    assert success, 'Class MinMax_v3 failed! Found min = %g != 0.0' % (minimum[0])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == 'test_MinMax_v1':
        ...
        test_MinMax_v2()
    elif len(sys.argv) >= 2 and sys.argv[1] == 'test_MinMax_v3':
        test_MinMax_v3()

__all__ = ['_find_extrema', '_refine_extrema', '_make_derivative', 'Differentiate', '_bisection', 'test_MinMax_v1', 
           'test_MinMax_v2', 'test_MinMax_v3','MinMax_v1', 'MinMax_v2', 'MinMax_v3']